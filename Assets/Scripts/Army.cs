...
public class Army:MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (next >= 0)
        {
            v = DirectionInPoint(pos, nextCell, path[next]);
        }
        else
            v = endPath - pos;
        v.Normalize();
        needAngle = Vector3.SignedAngle(transform.forward, new Vector3(v.x, 0, v.y), Vector3.up);

        pos += Date.dayPerSecond * Time.deltaTime * v * speed;
    ...
    public static Army CreateArmy(Region home,List<Regiment> list)
    {
        ...
        GameObject def = Instantiate(Main.that.ArmyPrefab[(int)state.fraction]);
        def.transform.position = MapMetrics.GetCellPosition(home.Capital);
        Army army = def.GetComponent<Army>();
        state.army.Add(army);
        army.InitArmy(list, home);
        ArmyBar bar = Instantiate(Main.that.ArmyBarPrefab, Main.that.mainCanvas);
   ...
   public bool TryMoveTo(Vector3 p,bool retreat = false)
    ...
    void BuildWayPoints(Vector2 begin, List<Vector2Int>way,Vector2 end)
    {
        ...
        for (int i=n-3;i>=0;)
        {
            point = 0 <= WayPointsIndex? WayPoints[WayPointsIndex--]: Instantiate(Main.that.WayPoint);
